...
static void decryptDBUS(void) {
    RC_Ctl.channel0 = (uint16_t) (((rxbuf[0]) | (rxbuf[1] << 8)) & 0x07FF);
    RC_Ctl.channel1 = (uint16_t) (((rxbuf[1] >> 3) | (rxbuf[2] << 5)) & 0x07FF);
    RC_Ctl.channel2 = (uint16_t) (((rxbuf[2] >> 6) | (rxbuf[3] << 2) | ((uint32_t) rxbuf[4] << 10)) & 0x07FF);
    RC_Ctl.channel3 = (uint16_t) (((rxbuf[4] >> 1) | (rxbuf[5] << 7)) & 0x07FF);
    RC_Ctl.s1 = (uint8_t) (((rxbuf[5] >> 4) & 0x000C) >> 2);
    RC_Ctl.s2 = (uint8_t) ((rxbuf[5] >> 4) & 0x0003);
}
